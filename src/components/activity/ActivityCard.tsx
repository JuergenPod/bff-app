  ...
  return (
    <>
      <motion.div
        ...
        className="relative bg-white/60 backdrop-blur-md border border-white/40 rounded-2xl overflow-hidden shadow-md cursor-pointer"
        onClick={() => onView(activity)}
      >
        {/* Cover photo or emoji banner */}
        <div className={cn("h-36 flex items-center justify-center", !hasCover && "bg-gradient-to-br from-rose-100 via-violet-100 to-amber-100")}>
          {hasCover ? (
        ...
        <div className="p-4">
          <div className="flex items-start justify-between mb-2">
            <h3 className="font-semibold text-gray-800 line-clamp-2 flex-1 text-sm leading-snug pr-2">
              {activity.title}
            </h3>
            <button
              type="button"
          ...
          {showMenu && (
            <div
              className="absolute right-4 top-44 bg-white rounded-xl shadow-xl border border-gray-100 z-10 py-1 min-w-28"
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-400">{activity.date ? formatDate(activity.date) : "No date"}</span>
